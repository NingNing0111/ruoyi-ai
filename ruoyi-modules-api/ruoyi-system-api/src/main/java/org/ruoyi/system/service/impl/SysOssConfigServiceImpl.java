...
public class SysOssConfigServiceImpl implements ISysOssConfigService {
	...
	@Override
	public void init() {
		List<SysOssConfig> list = TenantHelper.ignore(() -> baseMapper
			.selectList(new LambdaQueryWrapper<SysOssConfig>().orderByAsc(TenantEntity::getTenantId)));
	...
	public Boolean deleteWithValidByIds(Collection<Long> ids, Boolean isValid) {
		...
		if (flag) {
			list.forEach(sysOssConfig -> CacheUtils.evict(CacheNames.SYS_OSS_CONFIG, sysOssConfig.getConfigKey()));
		}
		return flag;
	}
	...
	private boolean checkConfigKeyUnique(SysOssConfig sysOssConfig) {
		...
		SysOssConfig info = baseMapper.selectOne(
				new LambdaQueryWrapper<SysOssConfig>().select(SysOssConfig::getOssConfigId, SysOssConfig::getConfigKey)
