...
public class SysDictTypeServiceImpl implements ISysDictTypeService, DictService {
	...
	private LambdaQueryWrapper<SysDictType> buildQueryWrapper(SysDictTypeBo bo) {
		...
		lqw.between(params.get("beginTime") != null && params.get("endTime") != null, SysDictType::getCreateTime,
	...
	public void deleteDictTypeByIds(Long[] dictIds) {
		for (Long dictId : dictIds) {
			...
			if (dictDataMapper
				.exists(new LambdaQueryWrapper<SysDictData>().eq(SysDictData::getDictType, dictType.getDictType()))) {
	...
	public List<SysDictDataVo> updateDictType(SysDictTypeBo bo) {
		...
		dictDataMapper.update(null,
				new LambdaUpdateWrapper<SysDictData>().set(SysDictData::getDictType, dict.getDictType())
	...
	public boolean checkDictTypeUnique(SysDictTypeBo dictType) {
		boolean exist = baseMapper
			.exists(new LambdaQueryWrapper<SysDictType>().eq(SysDictType::getDictType, dictType.getDictType())
	...
	public String getDictLabel(String dictType, String dictValue, String separator) {
		// 优先从本地缓存获取
		List<SysDictDataVo> datas = (List<SysDictDataVo>) SaHolder.getStorage()
			.get(CacheConstants.SYS_DICT_KEY + dictType);
